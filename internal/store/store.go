...
//go:generate mockgen -destination=../mocks/mock_store.go -package=mocks atlas-cli-plugin-test/internal/store CredentialsGetter
...
func (s *Store) createClustersClient(client *http.Client) error {
	...
	if s.baseURL != "" {
		opts = append(opts, atlasClustersPinned.UseBaseURL(s.baseURL))
	}
	c, err := atlasClustersPinned.NewClient(opts...)
	if err != nil {
		return err
	}
	s.clientClusters = c
